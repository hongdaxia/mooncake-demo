...
1. **传输协议取决于有没有 RDMA(InfiniBand) 网卡**：没有 RDMA 的机器统一用 **TCP**（`MOONCAKE_PROTOCOL=tcp`）；
   有 RDMA 的机器把它改成 `rdma`，即可获得零拷贝、超高带宽。
2. **GPU 使用范围可按需调整**：`env.sh` 里默认通过 `CUDA_VISIBLE_DEVICES=0,1,2,3` 只用前 4 张卡，
   你可以根据自己机器的空闲情况修改这一行。
